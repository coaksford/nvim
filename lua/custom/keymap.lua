...
map( 'n', '<leader>se', ts.diagnostics, { desc = '[S]earch [E]rrors' })
...
map( 'n', '<leader>f', function() ts.live_grep(ts_dropdown_theme) end, { desc = '[f]uzzily grep all files' })
map( 'n', '<leader>F', function() ts.current_buffer_fuzzy_find(ts_dropdown_theme) end, { desc = '[F]uzzily search in current buffer' })
